# HOPE chain-identity security contract
...
HOPE is hybrid Proof of Work + Proof of Stake. The modern HOPE block-production path must therefore refuse both PoW and PoS/coinstake production unless the active chain is proven to be the canonical HOPE chain.

## Known immutable identity

- Genesis: `00000270333dc42277a321c31bf0107115d8d6b551279e4c13c187a36ec6e133`
...
1. the security policy is explicitly qualified for production;
...
The guard must protect the central block-building path shared by PoW and PoS, not merely a launcher or one RPC method.

## Fail-closed configuration rule

`migration/modern-hope-draft/src/chain_identity_security.h` deliberately leaves the block-1 fingerprint, buried anchor and minimum-chainwork values unset and the policy unqualified.
...
Negative test: present a disposable wrong/truncated/alternate HOPE chain and prove that both PoW and PoS production are refused and the tip does not change.

Positive test: present a disposable copy of the genuine HOPE chain, prove the configured identity, and create valid PoW and PoS candidates/templates without solving/submitting a production block.

Only after both paths pass may the modernised HOPE code be considered for release.
