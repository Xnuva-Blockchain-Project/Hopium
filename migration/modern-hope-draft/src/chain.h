...
enum BlockStatus : uint32_t {
    ...
    // HOPE LEGACY DISK/CONSENSUS METADATA: preserve historical bit assignments.
    BLOCK_PROOF_OF_STAKE     =   128,
    BLOCK_STAKE_ENTROPY      =   256,
    BLOCK_STAKE_MODIFIER     =   512,

    // Modern-only flags are moved upward to avoid colliding with legacy HOPE bits.
    BLOCK_OPT_WITNESS        =  1024,
    BLOCK_STATUS_RESERVED    =  2048,
};
...
public:
    ...
    bool IsProofOfStake() const EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
    {
        AssertLockHeld(::cs_main);
        return (nStatus & BLOCK_PROOF_OF_STAKE) != 0;
    }

    bool IsProofOfWork() const EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
    {
        return !IsProofOfStake();
    }

    void SetProofOfStake() EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
    {
        AssertLockHeld(::cs_main);
...
/** Return the most recent block index of the requested proof type. */
