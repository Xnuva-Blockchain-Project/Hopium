...
void AddCoins(CCoinsViewCache& cache, const CTransaction &tx, int nHeight, bool check_for_overwrite) {
    ...
    for (size_t i = 0; i < tx.vout.size(); ++i) {
        const bool overwrite = check_for_overwrite ? cache.HaveCoin(COutPoint(txid, i)) : fCoinbase;
        // HOPE persists the creating transaction's timestamp/coinstake metadata
        // so future stake validation does not require a full transaction index.
        cache.AddCoin(COutPoint(txid, i),
                      Coin(tx.vout[i], nHeight, fCoinbase, fCoinStake, tx.nTime),
                      overwrite);
    }
}
