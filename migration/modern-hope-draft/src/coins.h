 ...
 * A UTXO entry.
 *
 * HOPE migration chainstate format:
 * - VARINT((height << 2) | (coinstake ? 2 : 0) | (coinbase ? 1 : 0))
 * - VARINT(transaction nTime)
 * - the non-spent CTxOut (via TxOutCompression)
 *
 * This is a NEW CHAINSTATE DATABASE FORMAT, not a new blockchain format.
 * The modern HOPE client must rebuild/reindex chainstate from the existing
 * HOPE block history rather than trying to reinterpret an old Bitcoin-style
 * UTXO database.
 */
class Coin
{
public:
    ...
    //! whether containing transaction was a coinbase / coinstake
    ...
    //! HOPE LEGACY STAKE METADATA: transaction timestamp used by the stake kernel.
    uint32_t nTime{0};

    //! Compatibility constructors used by generic modern code.
    ...
    //! HOPE-aware constructors used while connecting real HOPE transactions.
    ...
    void Serialize(Stream &s) const {
        assert(!IsSpent());
        const uint32_t code = nHeight * uint32_t{4} +
                              (fCoinStake ? uint32_t{2} : uint32_t{0}) +
                              (fCoinBase ? uint32_t{1} : uint32_t{0});
        ::Serialize(s, VARINT(code));
        uint32_t time = nTime;
        ::Serialize(s, VARINT(time));
