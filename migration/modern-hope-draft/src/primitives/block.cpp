...
{
    // Historical HOPE Scrypt hashes the canonical 80-byte serialized header.
    DataStream stream;
    stream << *this;
    assert(stream.size() == 80);

    uint256 out;
    hope::Scrypt1024_1_1_256(stream.data(), out.data());
    return out;
}

uint256 CBlockHeader::GetHash() const
{
    // Historical HOPE changed the block-id hash at block version 7.
