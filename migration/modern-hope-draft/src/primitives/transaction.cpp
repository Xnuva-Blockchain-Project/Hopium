...
CMutableTransaction::CMutableTransaction()
    : version{CTransaction::CURRENT_VERSION},
      nTime{static_cast<uint32_t>(TicksSinceEpoch<std::chrono::seconds>(NodeClock::now()))},
      nLockTime{0}
{
}
CMutableTransaction::CMutableTransaction(const CTransaction& tx)
    : vin(tx.vin), vout(tx.vout), version{tx.version}, nTime{tx.nTime}, nLockTime{tx.nLockTime}
{
}
...
{
    std::string str;
    str += strprintf("CTransaction(hash=%s, ver=%u, nTime=%u, vin.size=%u, vout.size=%u, nLockTime=%u)\n",
