...
public:
    ...
    // HOPE LEGACY CONSENSUS: coinstake marks its first output as empty.
    void SetEmpty()
    {
        nValue = 0;
        scriptPubKey.clear();
 ...
 * - uint32_t version
 * - uint32_t nTime (HOPE legacy consensus field when version < 2)
...
void UnserializeTransaction(TxType& tx, Stream& s, const TransactionSerParams& params)
{
    ...
    s >> tx.version;
    // HOPE LEGACY CONSENSUS: transaction versions below 2 serialize nTime
    // immediately after the version field. This is part of historical tx hashes.
    if (tx.version < 2) {
        s >> tx.nTime;
    } else {
        tx.nTime = 0;
...
void SerializeTransaction(const TxType& tx, Stream& s, const TransactionSerParams& params)
{
    ...
    s << tx.version;
    // HOPE LEGACY CONSENSUS: preserve historical version<2 transaction timestamp.
    if (tx.version < 2) {
        s << tx.nTime;
    }
    unsigned char flags = 0;
...
public:
    ...
    // HOPE LEGACY CONSENSUS: existing HOPE transactions default to version 1,
    // which carries nTime in the serialized transaction.
    ...
    // HOPE LEGACY CONSENSUS: serialized for transaction version < 2.
...
public:
    ...
    // HOPE LEGACY CONSENSUS: coinstake is identified by a non-coinbase input
    // and an empty first output followed by at least one staking output.
